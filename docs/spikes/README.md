...
| 3b | *Phase 3, not §6.1*: loop joins — the wrap, `setLooping`, the placed boundary | **The wrap wins at every configuration**, by 5.5× to 23 000× in damage energy; 0 join error everywhere; `setLooping` on a non-looping clip never comes back; a placed boundary costs a fixed 25–33 samples, which is Tracktion's own stop fade | [spike03b-loop-joins.md](spike03b-loop-joins.md) |
