...
| ~~An OSCQuery client~~ — **the desktop client needs none** (2026-09-17) | §9's question E was settled IN PROCESS, so the client reads `ParameterTree::snapshot()` and writes through `Engine::submit` — no HTTP poll, no WebSocket, no second copy of the tree. `src/wfg/engine/oscquery/OscQueryClient.h` stays what it was written for, Phase 2's mount probe, and its capitals stand unamended: *it blocks, and MountProbe owns the thread it runs on*. That sentence was going to need the words *inside the engine* to let a second process call it; it does not now. WFS-DIY's `Plugin/Source/Shared/OscQueryClient.{h,cpp}` remains the reference for the `?HOST_INFO` re-encoding trap, for the MOUNT side |
