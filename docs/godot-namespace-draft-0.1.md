...
**Draft 0.1** — the PRD §9.3 "parameter-tree namespace and node metadata schema", written
for the author's approval before anything under `/godot` is built (devplan, Phase 1,
*Needs from the author*). It also fixes the show-document schema, because the document is
the other half of the same namespace: every node under `/godot/cues` is a projection of an
attribute in `show.xml`.

Everything here is a proposal until approved. Items marked **Q** are questions the plan
could not answer from the PRD; each has a recommendation. Nothing marked *(proposed)* in
the PRD is touched.
...
1. **Objects are identity-addressed.** A cue lives at `/godot/cues/<id>` whatever list or
...
| Node | Type | Access | Meaning |
|---|---|---|---|
| `/godot/lists/order` | `s` | ro | list IDs in order, space-separated |
| `/godot/lists/focus` | `s` | rw | ID of the focused list; exactly one; write = `list.focus` |
| `/godot/lists/<id>/name` | `s` | rw | |
| `/godot/lists/<id>/order` | `s` | ro | IDs of the list's top-level children in order |
| `/godot/lists/<id>/standby` | `s` | rw | cue ID, or empty; write = `standby.set` |
...
### 2.4 `/godot/cues` — every cue and every group, flat, by ID

| Node | Type | Access | Meaning |
|---|---|---|---|
| `/godot/cues/<id>/kind` | `s` | ro | `memo` \| `group` (Phase 1 has no media; `memo` is a cue with no output) |
| `/godot/cues/<id>/number` | `s` | rw | the decimal cue number, a *string* (`12`, `12.5`, `12.5.1`), mutable, human protocol (§3.5) |
| `/godot/cues/<id>/name` | `s` | rw | |
| `/godot/cues/<id>/notes` | `s` | rw | |
| `/godot/cues/<id>/enabled` | `T`/`F` | rw | |
| `/godot/cues/<id>/colour` | `s` | rw | `#RRGGBB`; decoration, never the sole carrier (§4.8) |
| `/godot/cues/<id>/parent` | `s` | ro | ID of the containing list or group |
| `/godot/cues/<id>/index` | `i` | ro | position among siblings, 0-based |
| `/godot/cues/<id>/order` | `s` | ro | groups only: child IDs in order |
| `/godot/cues/<id>/mode` | `s` | rw | groups only: `timeline` \| `sequence` (§3.6) |
| `/godot/cues/<id>/advance` | `s` | rw | groups only: `auto` \| `manual` |
| `/godot/cues/<id>/preWait` | `d` | rw | seconds |
| `/godot/cues/<id>/postWait` | `d` | rw | seconds |

Groups carry no outputs, media or parameters (§4.12); Phase 1 stores their structure and
attributes so fixtures can nest, and nothing more. **Q4** below fixes what `standby.next`
does with them.
...
| Node | Type | Access | Meaning |
|---|---|---|---|
| `/godot/mounts/<id>/prefix` | `s` | ro | e.g. `/wfs` |
| `/godot/mounts/<id>/transport` | `s` | ro | `udp` \| `tcp` \| `ws` — declared now, used from Phase 2 (**Q7**) |
| `/godot/mounts/<id>/namespace` | `s` | ro | bundle-relative file, e.g. `namespaces/wfs-diy.json` |
| `/godot/mounts/<id>/rateCap` | `d` | ro | Hz, default for the mounted nodes |
| `/godot/mounts/<id>/anticipatable` | `T`/`F` | ro | default `F` for third parties (§3.3) |
| `/godot/mounts/<id>/panic` | `s` | ro | mount-level default (**Q7**) |
| `/godot/mounts/<id>/loaded` | `T`/`F` | ro | |
| `/godot/mounts/<id>/nodeCount` | `i` | ro | |

The mounted namespace itself appears at the prefix (`/wfs/…`), not under `/godot/mounts`.
...
| `node.touch` | `/godot/cmd/node/touch` | `s` address | per origin (**Q5**) |
...
- A value change on a listened node → one binary OSC message per tick, at most, to each
  ...
  currently touching that node (**Q5**).
- `cue.create`/`group.create` → `PATH_ADDED /godot/cues/<id>`; `*.delete` →
...
- `cue.move` and reorders → value pushes on `order`/`index`/`parent`; no `PATH_*`, since the
  flat `/godot/cues` container did not change.
...
MyShow/
  ...
  state.xml            ephemeral engine state (Q2)
...
name, attributes at their default omitted, numbers written by JUCE's classic-locale
formatter with the attribute's decimal cap (**Q1**), booleans `true`/`false`.

### 6.2 `show.xml`

| Element | Attributes (type, default) | Children |
|---|---|---|
| `Show` | `formatVersion` int (1) | `Lists`, `Mounts`, `Retired` (Q3) |
| `Lists` | — | `List*` |
...
A 12 1 ws:192.168.1.20:51234 node.set s:"/godot/cues/B3N8R5TW/name" s:"House to half"
A 12 2 ws:192.168.1.20:51234 cue.create s:"7K2QM9X4" i:3 s:"memo" s:"Blackout" s:"H5KT9WXF"
R 13 3 udp:192.168.1.7:9000 read-only node.set s:"/godot/cues/B3N8R5TW/kind" s:"group"
...
- **Q1 — Number precision.** Per-attribute decimal caps declared in the Schema
  (recommended: seconds 6, Hz 3, positions 4 when they arrive; the literal "fixed
  precision" of §3.20, which also keeps float32 noise such as `0.100000001490116` out of
  the file), or "as many decimal places as necessary" everywhere?
- **Q2 — Standby persistence.** Does standby survive save/load (in `state.xml`, as drawn
  above) or start empty on every load? §3.20 lists ephemeral state without naming standby;
  §3.5 calls it engine state.
- **Q3 — Tombstones.** "IDs are never reused" (§3.5) is only *verifiable* if deleted ids are
  kept. Recommended: the `Retired` element (8 bytes per deleted object), which also lets a
  Choufleur pointer show "deleted" rather than "unknown" (§3.23).
- **Q4 — Groups in Phase 1.** Schema and structure commands include groups so fixtures can
  nest, but `standby.next` treats a group as an opaque sibling and never descends; Phase 3
  defines traversal (§3.5, §3.6). Confirm the exclusion.
- **Q5 — Touch gating.** `node.touch`/`node.release` per origin; pushes to the touching
  origin suppressed while held; one push on release; release on disconnect. §3.16 makes
  touch gating a day-one engine concern; confirm the shape before any surface exists.
- **Q6 — Where the desktop UI runs.** In-process or a separate client process? Not
  load-bearing for Phase 1, but it decides whether the engine's message thread can be
  assumed idle, which Phase 2's plugin-parameter handover will want to know.
- **Q7 — Mount declaration.** `transport` declared now (used in Phase 2) and a mount-level
  `panic` default with per-node overrides in the namespace file — confirm the attribute set.
...
a cue's outputs and parameters (Phase 2+) become further nodes under `/godot/cues/<id>`,
